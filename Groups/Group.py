...
class Group:
    ...
    def updateProximity(self):
        max_proximity = 1
        min_proximity = 0.5
        #IMPORTANT: EDIT MIN MAX PROXIMITY IN GETPROXIMITY TOO
        grid_linearSpace = True

        if grid_linearSpace:
            self.proximity = np.random.rand(len(self.persons), 2) # 0 to 1 position coordinates
            proximity_range = (max_proximity - min_proximity)
            grid_sidelen = proximity_range / math.sqrt(2)
            self.proximity = self.proximity * grid_sidelen # 0 to grid_sidelen position coordinates
    ...
    def getProximity(self,p1,p2):
        max_proximity = 1
        min_proximity = 0.5
        #IMPORTANT: EDIT MIN MAX PROXIMITY IN UPDATEPROXIMITY TOO
        grid_linearSpace = True

        if grid_linearSpace:
            proximity_range = max_proximity - min_proximity
            point1 = self.proximity[self.person_mapper[p1]]
            point2 = self.proximity[self.person_mapper[p2]]
            dist = (point1[0]-point2[0])*(point1[0]-point2[0]) + (point1[1]-point2[1])*(point1[1]-point2[1])
            dist = math.sqrt(dist)
            prox = proximity_range - dist
            prox += min_proximity
            return prox
        else:
